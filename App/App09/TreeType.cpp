...
int Imp_LeafCount(TreeNode* tree){
  if (tree == NULL)
    ...
    return Imp_LeafCount(tree->right) + Imp_LeafCount(tree->left);
}

void Imp_DeleteLeaf(TreeNode*& tree){
  if (tree->left == NULL && tree->right == NULL){
    ...
    if (tree->left != NULL)
      Imp_DeleteLeaf(tree->left);
    if (tree->right != NULL)
      Imp_DeleteLeaf(tree->right);
  }
}
void TreeType::DeleteLeaf(){
