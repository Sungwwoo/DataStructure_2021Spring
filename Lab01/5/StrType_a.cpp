...
RelationType StrType::ComparedTo(StrType& otherString){
  ...
  cmpResult = strcmp(letters, otherString.letters);

  if (cmpResult < 0)       // the first character that does not match has a lower value in self than otherString
    return LESS;
  else if (cmpResult > 0)  // the first characterthat does not match has a greater value in self than otherString
    return GREATER;
  else                     // the contents of both strings are equal
    return EQUAL;  
